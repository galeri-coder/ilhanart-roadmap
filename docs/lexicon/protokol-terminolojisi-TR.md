...
## 🧩 Üyelik Seviyeleri — “Primer → Texture → Impasto”

| Seviye | Tanım | Teknik Temel |
|:--|:--|:--|
| **Primer (Astar)** | Giriş seviyesi. Temel doğrulamaları yapmış ama henüz zaman sürtünmesi (TWAB) biriktirmemiş üye. | `0 < TWAB ≤ 10⁰` |
| **Texture (Doku)** | Aktif katılımcı. Zamanla bakiye biriktirmiş, orta vadeli sadık üye. | `10⁰ < TWAB ≤ 10²` |
| **Impasto (Katman)** | En yüksek TWAB skoruna sahip, 365 günlük döngüleri tamamlamış kemik üye. | `TWAB > 10²` |
...
- **Artist (Sanatçı):** [PoArt] için Evidence Pack üretir, kayıt başlatır, yıllık doğrulama (heartbeat) imzasını atar.  
...
- **Fonksiyon:** Yıllık yenilemede (Evidence Pack) üzerinden doğrulama.  
> **v1.0 Değişmezlik Notu:** 7 gün ve 30 gün parametreleri v1.0 içinde hard-coded’ dır.

---

### 3.2) TWAB (Time-Weighted Average Balance)

TWAB, bir cüzdanın ekosistem içindeki **sadakat** ve **istikrar** katsayısını hesaplayan temel metriktir.
...
> **Simülasyon:**  
> Gerçek zamanlı olarak logaritmik oy gücü hesaplamasını görmek veya kendi TWAB senaryonuzu test etmek için [PoArt Simulation Console](https://galeri-coder.github.io/ilhanart-protocol/[PoArt]/) bağlantısına gidin.
...
Her yıl (Evidence Pack) üzerinden doğrulama zorunludur.
...
> Opsiyonel hızlandırma: Eğer proje “yıllık arşiv + düzenli denetim” altyapısına sahipse heartbeat periyodu (ör. 30 gün) teknik olarak düşürülebilir.  
...
- **Quorum (Minimum Katılım):** Toplam aktif oy ağırlığının en az **%25’i** katılmalı.  
- **Veto Threshold:** Veto, yalnızca katılanların %40’ı değil; tercihen **toplam aktif ağırlığın %40’ına** ulaşınca çalışır.  
...
Evidence Pack tutarsızdır. Topluluk, logaritmik skorlama ve TWAB etkisi ile %40 veto barajını geçerse kayıt asla mühürlenemez.
...
1. **Fallback Council** (en yüksek Impasto %10’u) geçici karar yetkisi alır.  
2. Karar, `Council Consensus ≥ ⅔` oranıyla alınır.  
3. İlk fırsatta (≥30 gün) topluluk referandumu ile onaylanmazsa otomatik olarak feshedilir.  
...
1) **İmpasto (≥100k):** anayasa, fee, strateji  
2) **Texture (50k–99k):** kürasyon, denetim, oylamalar  
3) **Primer (<50k):** öneriler, küçük ölçekli kararlar  
...
- **Anti-Bot / Anti-Sybil:** (uygulama katmanı) Solana-gated / stake-gated doğrulama  
